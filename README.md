...
I used essentially the same lab in previous years, and [[UMMCSci.CLabNotes][posted some comments]] based on the work they turned in. Some of these might be helpful, so you might want to have a look. They tend to come down to "don't forget how to program cleanly just because you're writing C". So don't!
...
%ENDCODE%
Run the resulting executable and verify that all six tests pass. %X% I would commit your code now __before__ you've changed anything. It make make it easier to see what you've done later.
...
that takes a null-terminated string, and returns a new null-terminated string (i.e., it doesn't mangle the original one) that contains the same characters in the same order, minus all the vowels.  Note that resulting array of characters will need to be allocated, and will typically be shorter than the input string.  It would be desirable to not waste memory and only allocate what you actually need for the return string; you might find valgrind useful for helping check for leaks.
...
%ENDCODE%
This is a destructive sorting operation, and should alter the array that it's given.  Note that since C doesn't know how large arrays are, we pass in the size as an argument.

To simplify the process, I've provided you with [[NicMcPhee.JavaImplementationsOfQuicksortAndMergesort][Java implementations of Quicksort and Mergesort]] that you can use as models.  Those implementations are [[NicMcPhee.JavaImplementationsOfQuicksortAndMergesort][here on the wiki]], and can also be found in =~mcphee/pub/CSci3401/SortingInJava=.  I strongly recommend you take advantage of these both because it will help ensure that you focus on the C issues on these problems and because it'll make them easier to grade.  (Having to figure out some crazy, unexpected approach is much more time consuming than reading a "standard" solution.)
...
_Note how inherently icky it is to have to pass all this bookkeeping information around, and how many wonderfully unpleasant errors can result from doing this incorrectly. It's a *lot* safer if arrays know how big they are._

=array_merge= should then generate a sorted list (small to large) of the unique values (i.e., no duplicates) in =values=.  Since we haven't yet learned how to return multiple values in C, we're going to use a slightly cheesy hack and return the =k= unique values in an array of size =k+1=, where the first element will hold the value =k=.  If, for example, there were 3 unique values (5, 8, and 9), then the result array would contain [3, 5, 8, 9].  The caller can then use the first element of the array to determine how long the array actually is.
...
You might also find your sorting algorithm from above useful in sorting your results.  With a little care, you can put the results in an unsorted array, and then ask your sorting algorithm to sort the desired bit of the array without messing with that important first element.
...
Remember to let us know what your group name is so that we can match names to groups on Github - you'll lose points if you don't. Thanks.
   ...
   * Jamie Miller and Geoff Schumacher - TeamDoctrine - Forkname: TeamDoctrine
   * Emma Callery and Eric Collom and Ian McGathey - Rhodium
   * Paul Schliep and Max Marti - TeamGoldWater
   * Max Magnuson and MK Dramdahl - SaveAndContinue
   * Alex Longerbone and Lincoln Kirkeide - Andrey_Dashkov
   * Ian Buck, Snuffy Linder - Akela
   * Emrakul Disciples = Alex Gunness & Travis Starkson & _Vladimir Putin & Vegeta & Ghost Nappa & PENGUINS_
   * David Pagels and Chris Thomas - NullPointerException
   * Andrew Stalock and Katie Reddemann - FlyingKittensOfAwesomeness
   * Alex Widdel and Braden Luthi - The_Judged
   * Chase Ottomoeller and Gabe Grimley - Moose_Rebels

-- Main.NicMcPhee - 19 Sep 2011<br>
-- Main.VincentBorchardt - 21 Jun 2012<br>
-- Main.lamberty - 18 Sep 2012
